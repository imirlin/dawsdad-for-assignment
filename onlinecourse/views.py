...
def show_exam_result(request, course_id, submission_id):
    ...
    score = 0
    is_got_score = False
    for i in submit.choices.all().filter(choice_is_correct=True).values_list('question_id'):
        is_got_score = is_got_score and Question.objects.filter(id=i[0]).first().is_get_score(selected)
        score += Question.objects.filter(id=i[0]).first().question_grade

    context['selected'] = selected
    context['question_grade'] = score
    context['is_got_score'] = is_got_score
